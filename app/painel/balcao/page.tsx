...
export default function BalcaoPage() {
  ...
  const missingFields = [
    !customerName.trim() && 'Nome do cliente',
    !customerPhone.trim() && 'Telefone',
  ].filter(Boolean) as string[]

  const handleFinishSaleClick = () => {
      ...
      <div className="w-full lg:w-96 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col">
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-gray-200 space-y-3">
            ...
            <button
              onClick={() => { setAttemptedSubmit(false); setShowCheckout(true) }}
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md animate-fade-in p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  className={`input-field ${attemptedSubmit && !customerName.trim() ? 'border-red-400 focus:border-red-500' : ''}`}
                  value={customerName}
                  onChange={(e) => setCustomerName(e.target.value)}
                  placeholder="Nome do cliente"
                  required
                />
                {attemptedSubmit && !customerName.trim() && (
                  <p className="text-xs text-red-600 mt-1">Informe o nome do cliente.</p>
                )}
              </div>
              <div>
                ...
                <input
                  type="tel"
                  className={`input-field ${attemptedSubmit && !customerPhone.trim() ? 'border-red-400 focus:border-red-500' : ''}`}
                  value={customerPhone}
                  onChange={(e) => setCustomerPhone(formatPhoneNumber(e.target.value))}
                  placeholder="(11) 99999-8888"
                  required
                />
                {attemptedSubmit && !customerPhone.trim() && (
                  <p className="text-xs text-red-600 mt-1">Informe o telefone.</p>
                )}
              </div>
              <div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4">
          {attemptedConfirm && missingGroups.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3 text-sm text-red-700 flex items-start gap-2">
              <AlertCircle size={16} className="flex-shrink-0 mt-0.5" />
              <span>Escolha uma opção em: <strong>{missingGroups.map((g) => g.name).join(', ')}</strong>.</span>
            </div>
          )}
          <button
